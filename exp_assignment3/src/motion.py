...
def get_random_position():
    ...
    randY = random.randint(-1,1) # y coordinate
    randPos = [randX, randY]
    return randPos
...
def move_random_normal():
	...
	act_c.send_goal(goalPos) # send position    	
	
	rospy.loginfo("NODE MOTION: Goal position is")
    	rospy.loginfo(goalPos)
	# wait for some time
    	#act_c.wait_for_result(rospy.Duration.from_sec(180.0))
	#result = act_c.get_result()
	
	wait = act_c.wait_for_result()
	if not wait:
		rospy.logerr("The Action server is not available")
	else:
		result = act_c.get_result()
		if result:
...
def main():
	...
	while not rospy.is_shutdown():			
		...
		if behaviour == "sleep":
			...
			if behaviour == "normal":
				rospy.loginfo("Execute move_random_normal() function")
                		move_random_normal()
                		rospy.sleep(1)
